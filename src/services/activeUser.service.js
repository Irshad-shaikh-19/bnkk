...
async function downloadAppleActiveUserAnalytics() {
  ...
  console.log("🔹 Searching for 'Metrics' tab...");
    ...
    const activeUsersPath = path.join(DOWNLOADS_DIR, "active_users.csv");

    // Remove existing file before saving
    if (fs.existsSync(activeUsersPath)) {
      fs.unlinkSync(activeUsersPath);
      console.log("🗑 Old active_users.csv removed.");
    }

    await download2.saveAs(activeUsersPath);
    console.log(`✅ CSV Downloaded: ${activeUsersPath}`);

    await browser.close();
    return activeUsersPath;
  
}

module.exports = {  downloadAppleActiveUserAnalytics };