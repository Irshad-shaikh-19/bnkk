...
async function downloadImpressionAnalytics() {
  ...
  console.log("🔹 Searching for 'Metrics' tab...");
    ...
    console.log("🔹 Selecting 'Impressions'...");
    await page.waitForSelector('text=Impressions', { timeout: 10000 });
    await page.click('text=Impressions');
    
    console.log("⏳ Waiting for the page to reload with Impressions data...");
    await page.waitForTimeout(3000); 
...
await exportMenuButton.waitFor({ state: 'visible', timeout: 35000 });
await exportMenuButton.click();
    ...
    const finalPath = path.join(DOWNLOADS_DIR, "impression.csv");
  
    // ✅ Remove existing file before saving
    if (fs.existsSync(finalPath)) {
      fs.unlinkSync(finalPath);
      console.log("🗑 Old impression.csv removed.");
    }
...
module.exports = { downloadImpressionAnalytics };